...
def load_and_preprocess_data(config) -> pd.DataFrame:
    ...
    # Map symptom names to criterion IDs
    pairs_df["criterion_id"] = pairs_df["DSM5_symptom"].map(symptom_to_criterion)

    unmapped_mask = pairs_df["criterion_id"].isna()
    if unmapped_mask.any():
        symptom_counts = pairs_df.loc[unmapped_mask, "DSM5_symptom"].value_counts().to_dict()
        console.print(
            "[yellow]⚠[/yellow] Dropping "
            f"{unmapped_mask.sum()} annotations with unmapped symptoms: {symptom_counts}"
        )
        pairs_df = pairs_df.loc[~unmapped_mask].copy()

    # Map symptom IDs to text
    ...
    pairs_df = pairs_df.dropna(subset=["post", "criterion", "criterion_id"])
