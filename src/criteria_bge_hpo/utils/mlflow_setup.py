...
def _make_absolute_tracking_uri(uri: str) -> str:
    """Normalize MLflow tracking URI to an absolute path when using local storage."""
    if uri.startswith("file:"):
        path = uri[len("file:") :]
        abs_path = to_absolute_path(path)
        return f"file:{abs_path}"

    # sqlite:///relative.db -> sqlite:////abs/path/relative.db
...
def setup_mlflow(config):
    ...
    tracking_uri = _make_absolute_tracking_uri(config.mlflow.tracking_uri)
