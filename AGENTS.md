...
Set up the toolchain with `python -m venv .venv && source .venv/bin/activate && pip install -e '.[dev]'`. Typical workflows: `python -m Project.cli train` for end-to-end K-fold training, `python -m Project.cli eval --fold 0` to sanity-check changes, and `python -m Project.cli hpo --n-trials 20` to spawn Optuna jobs. Run `ruff check src tests`, `black src tests`, `mypy src`, and `pytest` (optionally `--maxfail=1`) before committing.
